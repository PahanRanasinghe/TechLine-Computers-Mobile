...
export default function CustomerWarrantyScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setProductName('');
      setDescription('');
      loadMyClaims(); // Refresh history
    } catch (err) {
      Alert.alert('Submission Failed', err.response?.data?.message || 'Could not submit claim. Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const StatusBadge = ({ status }) => (
    <View style={[styles.statusBadge, { backgroundColor: (CLAIM_STATUS_COLORS[status] || '#6B7280') + '22' }]}>
      <Text style={[styles.statusText, { color: CLAIM_STATUS_COLORS[status] || '#6B7280' }]}>
        {status}
      </Text>
    </View>
  );
        ...
        <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.formCard}>
            ...
            <View style={styles.formIntro}>
              ...
              <Text style={styles.formTitle}>Warranty Claim</Text>
              <Text style={styles.formSubtitle}>
                Submit a request and our Customer Service team will review it shortly.
              </Text>
            </View>
            ...
            <View style={styles.formGroup}>
              ...
              <View style={styles.readonlyInput}>
                ...
                <View style={styles.lockBadge}><Text style={styles.lockText}>Auto-filled</Text></View>
              </View>
            </View>
            ...
            <View style={styles.formGroup}>
              ...
              <TextInput
                ...
                placeholder="Please describe exactly what is wrong with the product in detail..."
                placeholderTextColor="#4A5568"
                value={description}
            ...
            >
              {submitting ? (
                ...
                <Text style={styles.submitBtnText}>🛡️  Submit Claim</Text>
              )}
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.historySection}>
            <TouchableOpacity
              ...
              activeOpacity={0.8}
            >
              <Text style={styles.historyToggleText}>🧾  My Warranty Claims</Text>
              ...
              <View style={styles.historyBody}>
                {loadingClaims ? (
                  ...
                  <View style={styles.emptyHistory}>
                    ...
                    <Text style={styles.emptyHistoryText}>No warranty claims yet.</Text>
                  </View>
                ) : (
                  myClaims.map((claim) => (
                    <View key={claim._id} style={styles.claimCard}>
                      <View style={styles.claimHeader}>
                        <Text style={styles.claimCode}>{claim.productCode}</Text>
                        <StatusBadge status={claim.status} />
                      </View>
                      {claim.productName ? (
                        <Text style={styles.claimProductName}>{claim.productName}</Text>
                      ) : null}
                      <Text style={styles.claimDesc} numberOfLines={2}>{claim.issueDescription}</Text>
                      <Text style={styles.claimDate}>
                        Submitted: {new Date(claim.createdAt).toLocaleDateString('en-LK', {
...
const styles = StyleSheet.create({
  ...
  shieldIcon: { fontSize: 54, marginBottom: 10 },
  formTitle: {
    fontSize: 26,
    fontWeight: '800',
    color: C.text,
  ...
  lockText: { color: C.accent, fontSize: 10, fontWeight: '700' },
  ...
  claimCode: { color: C.accent, fontWeight: '800', fontSize: 14 },
  claimProductName: { color: C.text, fontSize: 13, fontWeight: '600', marginBottom: 4 },
  claimDesc: { color: C.muted, fontSize: 12, lineHeight: 18, marginBottom: 6 },
  claimDate: { color: '#374151', fontSize: 11 },

  statusBadge: { paddingHorizontal: 10, paddingVertical: 3, borderRadius: 8 },
  statusText: { fontSize: 11, fontWeight: '700' },

  // Footer
  footer: { alignItems: 'center', paddingVertical: 24, borderTopWidth: 1, borderColor: C.border, marginTop: 4 },
